...
open class RelativeIndex: IndexType {
	public init() {
	}

	open func materialize<C: Collection>(for collection: C) -> C.Index? {
...
public class EndIndex: RelativeIndex {
	override public func materialize<CollectionT: Collection>(for collection: CollectionT) -> CollectionT.Index? {
		return collection.endIndex
	}
}

public class OffsetIndex<Base: RelativeIndex>: RelativeIndex {
	let base: Base
	let offset: Int

	public init(base: Base, offset: Int) {
		self.base = base
		self.offset = offset
	}
...
public class FindIndex<Element: Equatable>: RelativeIndex {
	let value: Element
	...
	override public func materialize<CollectionT: Collection>(for collection: CollectionT) -> CollectionT.Index? where CollectionT.Element == Element {
		return collection.firstIndex(of: value)
	}
}

public class RelativeRange<Start: IndexType, End: IndexType> {
	let start: Start
	let end: End
	...
	public func materialize<CollectionT: Collection>(for collection: CollectionT) -> Range<CollectionT.Index>? {
		guard let start = start.materialize(for: collection), let end = end.materialize(for: collection), start <= end else {
			return nil
		}
...
public extension RelativeIndex {
	static let start = RelativeIndex()
	static let end = EndIndex()
}

public extension Collection {
	subscript<Start: RelativeIndex, End: RelativeIndex>(range: RelativeRange<Start, End>) -> SubSequence? {
...
let s = string[RelativeRange(start: .start, end: FindIndex(value: 5))]
print(s ?? "nil")
